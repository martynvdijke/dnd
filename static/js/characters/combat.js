...
export async function rollCheck(type, name, adv) {
    const c = currentChar;
    if (!c)
        return;
...
export async function applyHeal() {
    ...
    try {
        ...
        if (currentTab === 'combat') {
            const { renderCombat } = await import('./combat');
            renderCombat();
        }
        else {
            await loadOpenChar(c.id);
        }
    }
    catch (e) {
        toast(e.message, true);
...
export function autoSaveField(field, el) {
    if (autoSaveTimer)
...
export function renderStepper(field, value, delta, min, max, label, size) {

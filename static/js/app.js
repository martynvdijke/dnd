...
function renderCombat() {
    ...
    <div style="display:flex;gap:16px;margin-bottom:12px">
      <div><strong>Successes:</strong> ${[1, 2, 3].map(i => `<span class="slot-dot ${i <= (c.death_saves_successes || 0) ? 'filled' : 'empty'}" onclick="toggleDeathSave('successes',${i})" style="display:inline-block"></span>`).join('')}</div>
      <div><strong>Failures:</strong> ${[1, 2, 3].map(i => `<span class="slot-dot ${i <= (c.death_saves_failures || 0) ? 'filled' : 'empty'}" onclick="toggleDeathSave('failures',${i})" style="display:inline-block;background:${i <= (c.death_saves_failures || 0) ? 'var(--danger)' : 'transparent'};border-color:${i <= (c.death_saves_failures || 0) ? 'var(--danger)' : 'var(--border)'}"></span>`).join('')}</div>
    </div>
...
window.toggleDeathSave = async function (field, val) {
    if (!currentChar)
        return;

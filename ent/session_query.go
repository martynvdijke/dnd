...
func (_q *SessionQuery) QueryCharacter() *CharacterQuery {
...
func (_q *SessionQuery) WithCharacter(opts ...func(*CharacterQuery)) *SessionQuery {
	query := (&CharacterClient{config: _q.config}).Query()
...
//		GroupBy(session.FieldCharacterID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Session.Query().
//		Select(session.FieldCharacterID).
//		Scan(ctx, &v)
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withCharacter; query != nil {
		if err := _q.loadCharacter(ctx, query, nodes, nil,
			func(n *Session, e *Character) { n.Edges.Character = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadCharacter(ctx context.Context, query *CharacterQuery, nodes []*Session, init func(*Session), assign func(*Session, *Character)) error {
...
func (_q *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCharacter != nil {
			_spec.Node.AddColumnOnce(session.FieldCharacterID)
		}
	}

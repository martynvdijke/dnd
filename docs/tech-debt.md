...
| TD-005 | Edit UI | Several entities remain create/delete-only or lack any edit screen in the UI | NPC has `PUT /npcs/:id` (`handlers/routes_characters.go:74`) but no HTMX edit route (`handlers/htmx.go:274-279`) and no `PUT` caller in `ts/characters/npcs.ts`; `OneShotActNPC` (`handlers/routes_oneshot.go:33-35`), `CraftingRecipe` (`handlers/routes_characters.go:153-155`) and `FactionReputation` (`handlers/routes_misc.go:92-94`) expose no PUT; `DowntimeActivity` has full CRUD (`handlers/routes_characters.go:169-173`) but no `ts/` caller at all | Users can create but not edit these records, forcing delete+recreate | frontend | active | `edit-affordances` (follow-up to PR #135) |

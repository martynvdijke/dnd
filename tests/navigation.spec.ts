...
import { clickBottomTabForce, login, NAV_TIMEOUT } from './helpers.js';
...
test.describe('Navigation: Bottom Tab Bar & Sidebar', () => {
  ...
  test('bottom tab bar is visible on mobile at 390x844', async ({ page }) => {
    ...
    await expect(bottomBar).toBeVisible({ timeout: NAV_TIMEOUT });
  });
  ...
  test('bottom tabs change views', async ({ page }) => {
    ...
    await expect(page.getByTestId('compendium-view')).toBeVisible({ timeout: NAV_TIMEOUT });
  });
  ...
  test('More menu button exists on mobile', async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await expect(page.locator('#moreTabBtn')).toBeVisible({ timeout: NAV_TIMEOUT });
    await expect(page.locator('#moreTabBtn')).toContainText('More');
  });

  test('session mode topbar exists in DOM', async ({ page }) => {
    await expect(page.locator('#sessionModeTopbar')).toBeAttached({ timeout: NAV_TIMEOUT });
  });
});

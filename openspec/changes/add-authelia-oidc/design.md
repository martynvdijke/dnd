...
DND (Villum) is a Go + session-cookie app with local password auth (handlers/auth.go, middleware/auth.go). Homelab forward-auth via Authelia at https://authelia.vandijke.xyz causes double-login and breaks non-browser clients. This change makes DND (Villum) a native OIDC relying party against Authelia, aligning with /root/homelab/openspec/changes/authelia-oidc. Port: 6280, host: https://dnd.vandijke.xyz.
dnd already uses Google OAuth2 (google_calendar) for calendar sync — Authelia OIDC is for *login* only and must coexist. Google flow stays for calendar import; OIDC adds a separate provider.

...
interface EncounterMonster {
  ...
  hp: number;
  count: number;
  compendium_monster_id?: number;
  [key: string]: unknown;
}

// ─── Encounter Builder ───

expose('showEncounterBuilder', async function (): Promise<void> {
  showView('encounter');
  const el = document.getElementById('encounterContent')!;
  el.innerHTML = '<div class="ornament">✧ Loading encounters... ✧</div>';
  try {
    const encounters = await api<Encounter[]>('GET', '/api/encounters');
    el.innerHTML = `
  ...
  } catch (e: unknown) { el.innerHTML = `<div class="empty-state"><p class="small text-muted">Error: ${esc(e instanceof Error ? e.message : String(e))}</p></div>`; }
});

expose('showCreateEncounter', function (): void {
...
expose('saveEncounter', async function (): Promise<void> {
  ...
  hideModal();
  window.showEncounterBuilder();
  toast('Encounter created');
});

expose('showEncounterDetail', async function (id: number): Promise<void> {
  ...
  try {
    const e = await api<Encounter>('GET', `/api/encounters/${id}`);
    ...
    <div class="d-flex gap-2 flex-wrap">
      ...
      <div id="monsterList">
        ${monsters.length ? monsters.map((m) => `
          <div class="inv-item">
            <div>
    ...
    el.innerHTML = `<div class="empty-state"><p class="small text-muted">Error: ${esc(msg)}</p>
      <button class="btn btn-outline-secondary btn-sm" onclick="window.showEncounterBuilder()">Back</button></div>`;
  }
});

expose('editEncounter', function (_id: number): void { window.showEncounterBuilder(); });

expose('deleteEncounter', async function (id: number): Promise<void> {
...
expose('showEncounterMonsterPicker', async function (eid: number): Promise<void> {
...
expose('showEncounterMonsters', function (eid: number, cid: number): void {
...
expose('showEncounterList', function (cid: number): void {
  const card = document.getElementById('campaignEncountersSection') as HTMLElement | null;
  if (!card) return;
...
expose('showNewEncounterForm', function (_cid: number): void {
  showModal('New Encounter', `<div hx-get="/htmx/campaigns/${_cid}/encounters/new" hx-trigger="load" hx-swap="innerHTML"><div class="text-center py-3"><i class="fa-solid fa-spinner fa-spin me-1"></i>Loading...</div></div>`);
});

expose('editEncounterMonster', function (mid: number, eid: number, cid: number): void {
...
expose('deleteEncounterMonster', function (mid: number, eid: number, cid: number): void {
  if (!confirm('Remove this monster from the encounter?')) return;
  const card = document.getElementById('campaignEncountersSection') as HTMLElement | null;
  if (!card) return;
  window.htmx.ajax('DELETE', `/htmx/encounters/${eid}/monsters/${mid}?campaign_id=${cid}`, { target: card, swap: 'innerHTML' });
});

expose('showAddEncounterMonsterForm', async function (eid: number, cid: number): Promise<void> {
...
function showAddEncounterMonsterCustom(eid: number, cid: number): void {
...
expose('showCompendiumMonsterPicker', function (eid: number, _cid: number): void {
...
function refreshCampaignEncounters(_cid: number): void {
  const card = document.getElementById('campaignEncountersSection') as HTMLElement | null;
  if (card) window.htmx.trigger(card, 'load');
}

expose('importCompendiumMonsterToEncounter', async function (monsterId: number, encounterId: number, _campaignId?: number): Promise<void> {
  try {
    ...
    hideModal();
    window.showEncounterDetail(encounterId);
  } catch (e: unknown) {
    toast(e instanceof Error ? e.message : String(e), true);
  }
});

expose('deleteMonster', async function (eid: number, mid: number): Promise<void> {
  if (!confirm('Remove this monster?')) return;
  await api('DELETE', `/api/encounter-monsters/${mid}`);
  window.showEncounterDetail(eid);
  toast('Monster removed');
});

expose('showEncounterXPCalc', function (): void {
...
expose('doXPCalc', async function (): Promise<void> {
  const levels = (document.getElementById('xpPartyLevels') as HTMLInputElement).value.split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => !isNaN(n));
  ...
  const monsters = lines.map((l) => {
    ...
    return { cr, count: parseInt(count || '1', 10) || 1, name: 'Custom' };
  });
  if (!levels.length || !monsters.length) { toast('Enter party levels and monsters', true); return; }
  try {
    const result = await api<{ total_xp: number; adjusted_xp: number; difficulty: string; party_size: number; monster_count: number; size_multiplier: number; thresholds: { easy: number; medium: number; hard: number; deadly: number } }>('POST', '/api/encounters/calculate-xp', { party_levels: levels, monsters });
